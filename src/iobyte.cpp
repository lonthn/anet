...
/* Class byte_writer implementation */
...
void obstream::initialize(size_t capacity) {
    if (_buffer == nullptr) {
        resize(capacity);
    }
}
...
obstream& obstream::operator<<(GString& val) {
#ifdef _WIN32
    std::string tempVal = strutil::gbkToUtf8(val.c_str());
#else
...
obstream& obstream::operator << (const GString& val) {
#ifdef _WIN32
    std::string tempVal = strutil::gbkToUtf8(val.c_str());
#else
...
ibstream::ibstream(std::string &data) :
        ibstream((data.data()), (unsigned int) data.length()) {
}
...
void ibstream::seek(int offset, seek_origin origin) {
    ...
    if (_pointer < _begin) {
        DLOG(FATAL) << "-> Seek] Move pointer out range, pointer: "
                    << (int32_t)(_pointer-_begin)
                    << ", length: " << (int32_t) (_end-_begin)<<".";
...
void ibstream::read(void *data, size_t length) {
    if (!is_data_lack(self)) {
        if ((_pointer+length) > _end) {
            _pointer = _end + 1;
            DLOG(FATAL) << "-> Read] Read pointer out range, pointer: "
                        << (int32_t)(_pointer-_begin)
                        << ", length: " << (int32_t)(_end-_begin) << ".";
        } else {
...
ibstream& ibstream::operator >> (long long &val) {
    read(&val, 8);
    val = (long long) ntohll(val);
    return self;
}
ibstream& ibstream::operator >> (unsigned long long &val) {
    read(&val, 8);
    val = ntohll(val);
    return self;
}
...
ibstream& ibstream::operator >> (GString &val) {
    uint32_t length;
    ...
    if ((_pointer+length) > _end) {
        _pointer = _end + 1;
        DLOG(FATAL) << "'>> string' is data lack.";
        return self;
    }
...
GString ibstream::read_string() {
    ...
    GString ret((const char *) _pointer, length);
#ifdef _WIN32
